...
public interface ConsultationRepository extends JpaRepository<Consultation, Integer> {
    ...
    @Query(
            value = "SELECT ct FROM Consultation ct WHERE ct.medecin = ?1 AND ct.Status = ?2"
    )
    Optional<List<Consultation>> findByMedecinAndStatus(Medecin medecin, ConsultationStatus Status);
